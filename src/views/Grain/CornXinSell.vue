...
const TDS_CornXinSell = ref([])
const TDS_CornXinSellWeighRecord = ref([])
...
const ACT_PageQuery = () => {
    ...
    $Requests.post('/cornXinSell/pageQuery', QUERY_Main.value)
        .then(response => {
...
onMounted(() => {
    // 页面加载时获取表设计
    $TDS.setTableDesign(TDS_CornXinSell, 't_corn_xin_sell');
    $TDS.setTableDesign(TDS_CornXinSellWeighRecord, 'ts_corn_xin_sell_weigh_record');
    // 获取交易列表
    ACT_PageQuery()
})
...
const OPT_clearingForm = computed(() => {
    try {
        return TDS_CornXinSell.value.filter(item => item.columnName === 'clearingForm')[0].types
    } catch (error) {
        return {}
    }
});
...
const OPT_tradeStatus = computed(() => {
    try {
        return TDS_CornXinSell.value.filter(item => item.columnName === 'tradeStatus')[0].types
    } catch (error) {
        return []
    }
});
...
const OPT_impurity = computed(() => {
    try {
        return TDS_CornXinSell.value.filter(item => item.columnName === 'impurity')[0].types
    } catch (error) {
        return []
    }
});
...
const OPT_carrier = computed(() => {
    try {
        return TDS_CornXinSellWeighRecord.value.filter(item => item.columnName === 'carrier')[0].types
    } catch (error) {
        return {}
    }
});
...
const OPT_buyerType = computed(() => {
    try {
        return TDS_CornXinSell.value.filter(item => item.columnName === 'buyerType')[0].types
    } catch (error) {
        return []
    }
});
...
const OPT_weighSide = computed(() => {
    try {
        return TDS_CornXinSell.value.filter(item => item.columnName === 'weighSide')[0].types
    } catch (error) {
        return {}
    }
});

const DEALED_TDS_CornXinSell = computed(() => {
    // 插入补充字段
    const newTDS = TDS_CornXinSell.value.concat([
        {
...
// 过滤表设计
const FLDTDS_CornXinSell = computed(() => {
    if (loginUserStore.loginUser.roleCode == 'sysAdmin') {
        // 如果是系统管理员,则过滤掉敏感字段
        return DEALED_TDS_CornXinSell.value.filter(field => FLD_field.value.includes(field.columnName))
    }
...
const SBM_saveTrade = () => {
    ...
    FORM_Trade.value.validateField(['tradeDate', 'buyerType', 'weighSide', 'buyerPerson.phoneNum', 'buyerPerson.personName', 'buyerCompany.companyName',
        ...
        (valid, fields) => {
            ...
            // 提交保存出售
            $Requests.post('/cornXinSell/saveTrade', FD_Trade.value, { showSuccessMsg: true })
                .then(response => {
                    if (response.code === 200) {
...
const SBM_sellComplete = () => {
    ...
    ElMessageBox.confirm('提交完成前请仔细核对录入内容, 提交后数据将进入待结算状态？', '提示', {
        ...
        FORM_Trade.value.validate((valid, fields) => {
            ...
            $Requests.post('/cornXinSell/sellComplete', FD_Trade.value, { showSuccessMsg: true })
                .then(response => {
                    if (response.code === 200) {
                        SHOW_Drawer.value = false;
                        ACT_PageQuery();
...
const ACT_EditTrade = (row) => {
    // 查询交易详情
    $Requests.post('/cornXinSell/getTradeDetail', row)
        .then(response => {
            if (response.code === 200) {
...
const settleTrade = () => {
    // 提交结算
    $Requests.post('/cornXinSell/settleTrade', FD_Trade.value, { showSuccessMsg: true })
        .then(response => {
            if (response.code === 200) {
                SHOW_Drawer.value = false;
                ACT_PageQuery();
...
<template>
    <el-card class="page-container">
        <template #header>
            ...
            <div class="field-filter" v-if="SHOW_fieldFilter" v-click-outside="() => SHOW_fieldFilter = false">
                <el-checkbox-group v-model="FLD_field" style="display: flex; flex-wrap: wrap;">
                    <el-checkbox-button v-for="(field, index) in DEALED_TDS_CornXinSell" :key="index"
        ...
        <el-table :data="TD_List" style="width: 100%">
            ...
            <el-table-column v-for="field in FLDTDS_CornXinSell" :key="field.columnName" :label="field.columnComment"
